...
class Subgraphs(Dataset):
    def __init__(self, root, mode, subgraph2label, n_way, k_shot, k_query, batchsz, args, adjs, h):
        self.batchsz = batchsz  # batch of set, not batch of subgraphsy(一批集合)
        self.n_way = n_way  # n-way
        self.k_shot = k_shot  # k-shot support set（支持集）
        self.k_query = k_query  # for query set（查询集）
        self.setsz = self.n_way * self.k_shot  # num of samples per support set（每个支持集的样本数）
        self.querysz = self.n_way * self.k_query  # number of samples per set for evaluation（每组评估的样本数）
        ...
        # load subgraph list if preprocessed加载子图列表（如果已预处理）
